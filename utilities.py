...
# ============================================================
# Parameter metadata for colouring by network location
# ============================================================

def parameter_metadata(model):
    """
    Returns one metadata row per scalar parameter, in the same flattening order
    as model.named_parameters().
    """
    rows = []
    ...
    for name, p in model.named_parameters():
        ...
        if isinstance(module, nn.Linear):
            if module_name == f"net.{3 * cfg.n_hidden}":
                group = "final linear"
            elif module_name == "net.0":
                group = "input linear"
            else:
                group = "hidden linear"

            if param_type == "weight":
                group = f"{group} weight"
            elif param_type == "bias":
                group = f"{group} bias"

        elif isinstance(module, nn.LayerNorm):
            if param_type == "weight":
                group = "LayerNorm gamma"
            elif param_type == "bias":
                group = "LayerNorm beta"
            else:
                group = "LayerNorm"
