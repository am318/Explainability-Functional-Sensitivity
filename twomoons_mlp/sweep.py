"""
The two-moons experiment end to end: sweep learning rates for Adam, SGD and
SGD+momentum, pick the best learning rate for each, re-train those three
fully instrumented, and analyse how the parameter-wise sensitivity ordering
settles over training.

See common/sweep.py for why this runs in two passes (cheap selection, then
instrumented re-training of the winners only) and how "best" is defined.

Usage:
    python sweep.py
    python sweep.py --epochs 20 --probes 4      # quicker, coarser
    python sweep.py --output-root outputs/rerun
...
def main() -> None:
    ...
    run_sweep(
        ...
        device=device, title="Two moons MLP: learning-rate sweep", probes=args.probes,
    )
