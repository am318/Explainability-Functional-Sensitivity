...
logic is used by shakespeare_lstm/rank_stability.py).
...
(input/hidden/output) breakdowns are *only* shown at full resolution, since
the pooled heatmap allocates rows proportionally to parameter count and a
small module can collapse to too few pooled rows for any of these statistics
to mean anything. This model is small enough that "full resolution" is every
one of its 1,218 parameters at every epoch, so the two curves should agree
closely -- where they do not, the pooling is what differs.
...
Usage:
    python rank_stability.py outputs/best/adam_lr0.001
    python rank_stability.py outputs/best/adam_lr0.001 --probes 16
    python rank_stability.py outputs/best/adam_lr0.001 --recompute  # ignore cache
"""
...
def main() -> None:
    ...
    parser.add_argument("experiment_dir", type=str, help="e.g. outputs/best/adam_lr0.001")
