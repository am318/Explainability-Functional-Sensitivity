"""
ViT tiny SNIP pruning experiment.

This script performs zero-shot SNIP pruning at initialization, then trains the remaining ViT parameters from scratch. It uses a sensitivity-derived pruning budget, global SNIP mask selection, and the same checkpointed sensitivity analysis as the baseline ViT tiny script.
...
Example:
    EPOCHS=100 BATCH_SIZE=128 python ViT_Tiny_SNIP_Pruning.py
"""
...
class Config:
    ...
    output_dir: str = _env_str("OUTPUT_DIR", "Plots/vit_tiny_snip")
...
run_tag = f"{cfg.dataset.lower()}_vit_tiny_snip_{parameter_count(model)}_params"
summary = {
    "config": asdict(cfg),
    "device": str(device),
    ...
    "history": history,
    "analysis_note": "SNIP pruning uses a sensitivity-derived sparsity target and the same checkpointed analysis pipeline as the baseline ViT tiny script.",
}

summary_path = out_dir / "vit_tiny_snip_pruning_summary.json"
