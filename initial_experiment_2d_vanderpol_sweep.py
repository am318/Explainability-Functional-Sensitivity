...
# Allow sweep control from the environment so a launcher script can vary
# architecture and run isolation without editing the training code.
n_hidden = int(os.getenv("N_HIDDEN", cfg.n_hidden))
hidden_width = int(os.getenv("HIDDEN_WIDTH", cfg.hidden_width))
...
for epoch in range(epochs + 1):
    ...
    if epoch % checkpoint_interval == 0:
        ...
        if (S_ref is None) and (epoch >= compare_epoch):
...
ax.set_ylabel("v")
prettify_axes(ax)
save_pub_figure(fig, f"{output_dir}/True_Vector_Field_{parameter_count(model)}_Parameters_{n_hidden}_Depth_{hidden_width}_Width.pdf")
...
ax.set_ylabel("v")
prettify_axes(ax)
save_pub_figure(fig, f"{output_dir}/Learned_Vector_Field_{parameter_count(model)}_Parameters_{n_hidden}_Depth_{hidden_width}_Width.pdf")
...
save_pub_figure(fig, f"{output_dir}/Vector_Field_Error_{parameter_count(model)}_Parameters_{n_hidden}_Depth_{hidden_width}_Width.pdf")
...
save_pub_figure(fig, f"{output_dir}/Eigenspectrum_{parameter_count(model)}_Parameters_{n_hidden}_Depth_{hidden_width}_Width.pdf")
...
ax.yaxis.set_minor_formatter(matplotlib.ticker.NullFormatter())
beautify_legend(ax, loc="best")
save_pub_figure(fig, f"{output_dir}/Loss_Evolution_{parameter_count(model)}_Parameters_{n_hidden}_Depth_{hidden_width}_Width.pdf")
...
save_pub_figure(
    ...
    f"{parameter_count(model)}_Parameters_{n_hidden}_Depth_{hidden_width}_Width.pdf",
)
...
with open(f"{output_dir}/final_summary_2d_vanderpol_{parameter_count(model)}_Parameters_{n_hidden}_Depth_{hidden_width}_Width.json", "w") as f:
