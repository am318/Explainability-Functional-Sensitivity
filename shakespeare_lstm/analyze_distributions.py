...
"Before" is a fresh model built from the exact same seed the run started
from (i.e. its initialization, bit-for-bit reproducible since model init
only consumes the CPU RNG regardless of training device); "after" is the
run's final checkpoint. Both are scored on the same held-out probe set the
run used for its own tracking, so the only thing that differs between the
two distributions is the training that happened in between.
...
def main() -> None:
    ...
    before_model = CharLSTM(len(vocab), cfg.embedding_dim, cfg.rnn_units).to(device)

    ckpt = torch.load(ckpt_path, map_location=device)
    after_model = CharLSTM(len(vocab), cfg.embedding_dim, cfg.rnn_units).to(device)
