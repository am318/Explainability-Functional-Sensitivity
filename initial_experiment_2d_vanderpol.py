...
im1 = axes[1, 1].imshow(
    np.abs((C_final - C_init).detach().cpu().numpy()),
    aspect="auto",
    cmap="RdBu_r",
)
...
axes[2, 2].set_xlabel("early sensitivity mass fraction")
axes[2, 2].set_ylabel("final sensitivity mass fraction")
axes[2, 2].legend()

plt.tight_layout()
plt.savefig(f"Plots/Initial_Experiment_2D_VanderPol_{parameter_count(model)}_Parameters.pdf")
plt.close(fig)
