"""
Does WHEN you prune matter, for the same amount pruned? See
shakespeare_lstm/pruning_experiment.py for the full explanation -- same
experiment, on the WikiText-2 AWD-LSTM instead.

Usage:
    python pruning_experiment.py outputs/full_run_1
    python pruning_experiment.py outputs/full_run_1 --keep-fraction 0.2 \\
        --prune-epochs 0 10 20 30 40
"""
...
@torch.no_grad()
def evaluate(model, loader: DataLoader, criterion: nn.Module, device: torch.device) -> Tuple[float, float]:
    model.eval()
...
def train_one_masked_epoch(
    model, loader: DataLoader, optimizer: torch.optim.Optimizer, criterion: nn.Module,
    device: torch.device, masks: Dict[str, torch.Tensor], grad_clip: float, desc: str,
) -> float:
    model.train()
    loss_sum, n = 0.0, 0
...
def build_model_at_epoch(epoch: int, cfg: Config, vocab_size: int, device: torch.device, experiment_dir: Path):
    if epoch == 0:
        set_seed(cfg.seed)
        return build_model(cfg, vocab_size, device)
...
def run_branch(
    prune_epoch: int, final_epoch: int, cfg: Config, vocab_size: int, device: torch.device,
    experiment_dir: Path, train_loader: DataLoader, probe_loader: DataLoader, criterion: nn.Module,
    keep_fraction: float, n_probes: int, out_dir: Path,
) -> Dict:
...
def plot_results(
    ...
    ax_acc.set_ylabel("Held-out next-word accuracy")
...
def main() -> None:
    ...
    parser.add_argument("--prune-epochs", type=int, nargs="+", default=[0, 10, 20, 30, 40])
    ...
    baseline_model = build_model_at_epoch(final_epoch, cfg, vocab_size, device, experiment_dir)
    ...
    for prune_epoch in args.prune_epochs:
        result = run_branch(
            prune_epoch, final_epoch, cfg, vocab_size, device, experiment_dir,
