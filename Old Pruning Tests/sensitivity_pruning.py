...
def _snapshot_similarity_local(
    current: Dict[str, torch.Tensor],
    previous: Optional[Dict[str, torch.Tensor]],
) -> float:
    if not previous:
...
def sensitivity_prune(
    ...
    """
    Two-pass sensitivity pruning.

    Per round:
      1. Estimate sensitivities on the current working model.
      2. Build a first-pass threshold/structured mask.
      3. Apply that first-pass mask to a temporary probe model.
      4. Re-estimate sensitivities with the first-pass mask active.
      5. Build a second-pass threshold/structured mask using the same ranking rule.
      6. Intersect the first-pass, second-pass, and previous active masks.

    Return format matches build_structured_masks_iterative:
        final_masks, final_stats, final_embed_sel, final_hidden_sel, final_scores
    """
    import copy

    # Work on a copy so the caller's model is only modified once, at the end.
    working_model = copy.deepcopy(model)
    ...
    total_rounds = max(1, int(_cfg_value(cfg, "iterative_pruning_rounds", 1)))
    target_prune_fraction = float(_cfg_value(cfg, "prune_fraction", 0.0))

    round_fractions: List[float] = []
    round_first_pass_prune: List[float] = []
    round_second_pass_prune: List[float] = []
    round_intersection_prune: List[float] = []
    round_score_stability: List[float] = []
    round_probe_confidences_first: List[float] = []
    round_probe_confidences_second: List[float] = []

    early_stop_triggered = False

    for round_idx in range(total_rounds):
        if bool(_cfg_value(cfg, "gradual_sparsification", False)):
            round_prune_fraction = _iterative_prune_fraction(cfg, round_idx)
        else:
            round_prune_fraction = target_prune_fraction

        # Force the final configured round to use the requested target.
        if round_idx == total_rounds - 1:
            round_prune_fraction = target_prune_fraction

        round_prune_fraction = max(0.0, min(target_prune_fraction, float(round_prune_fraction)))
        round_fractions.append(round_prune_fraction)

        # ------------------------------------------------------------------
        # Pass 1: score the currently active working model.
        # ------------------------------------------------------------------
        scores_full, _, meta_full = compute_sensitivity_scores(
            working_model,
            loader,
            cfg,
            device,
        ...
        first_masks, first_stats, first_embed_sel, first_hidden_sel = _build_structured_selection_masks(
            working_model,
            scores_full,
            cfg,
            prune_fraction=round_prune_fraction,
        ...
        # Make the first pass cumulative before using it for the masked probe.
        for name, first in first_masks.items():
            if is_prunable_parameter(name, params[name], cfg):
                first_masks[name] = _intersect_and_keep_one(
                    first.to(device=params[name].device, dtype=torch.bool),
                    active_masks[name],
                    scores_full[name].to(device=params[name].device),
                )
            else:
                first_masks[name] = first.to(device=params[name].device, dtype=torch.bool)

        # ------------------------------------------------------------------
        # Pass 2: score a temporary copy with the first-pass mask active.
        # ------------------------------------------------------------------
        probe_model = copy.deepcopy(working_model)
        apply_masks_(probe_model, first_masks)

        scores_masked, _, meta_masked = compute_sensitivity_scores(
            probe_model,
            loader,
            cfg,
            device,
        ...
        # Use score stability between unmasked and first-pass-masked scoring
        # as a conservative confidence weight.
        stability_weights: Dict[str, float] = {}
        for name, masked_score in scores_masked.items():
            full_score = scores_full.get(name)
            sim = _vector_similarity(masked_score, full_score) if full_score is not None else None
            if sim is None:
                continue
            stability = max(0.0, min(1.0, 0.5 * (sim + 1.0)))
            conf = _as_confidence_scalar(
                meta_masked.get("parameter_confidence", {}).get(name),
                1.0,
            )
            stability_weights[name] = max(0.25, min(1.0, stability * conf))

        second_masks, second_stats, second_embed_sel, second_hidden_sel = _build_structured_selection_masks(
            probe_model,
            scores_masked,
            cfg,
            prune_fraction=round_prune_fraction,
        ...
        # ------------------------------------------------------------------
        # Final round mask: intersection of pass 1, pass 2, and active mask.
        # ------------------------------------------------------------------
        candidate_masks: Dict[str, torch.Tensor] = {}

        for name, p in params.items():
            if not is_prunable_parameter(name, p, cfg):
                candidate_masks[name] = torch.ones_like(p, dtype=torch.bool, device=p.device)
                continue

            first = first_masks[name].to(device=p.device, dtype=torch.bool)
            second = second_masks[name].to(device=p.device, dtype=torch.bool)
            previous = active_masks[name].to(device=p.device, dtype=torch.bool)

            intersected = first & second

            # Use the masked-model scores for rescue decisions, because these
            # reflect the post-first-pass functional landscape.
            rescue_score = scores_masked.get(name, scores_full[name]).to(device=p.device)
        ...
        # Apply connectivity repair after the explicit intersection. This is
        # necessary because intersecting two individually valid masks can still
        # break row/column connectivity constraints.
        restored_total = 0
        retained_eligible = 0
        eligible_total = 0
        ...
        with torch.no_grad():
            for name, p in params.items():
                ...
                if bool(_cfg_value(cfg, "connectivity_closure", False)) and p.ndim in (2, 4):
                    score = scores_masked.get(name, scores_full[name]).to(device=mask.device)
                    restored_total += connectivity_close_dense_weight(
                        mask,
                        score,
                        max(1, int(_cfg_value(cfg, "min_connections_per_unit", 1))),
                    )
                ...
                retained_eligible += int(mask.sum().item())

        actual_prune_fraction = float(
            max(0, eligible_total - retained_eligible) / max(1, eligible_total)
        )

        # Save round outputs.
        final_masks = candidate_masks
        final_embed_sel = second_embed_sel
        final_hidden_sel = second_hidden_sel

        # Conservative final score: a parameter is trusted only to the extent
        # that it remains sensitive both before and after the first-pass mask.
        final_scores = {}
        for name, s2 in scores_masked.items():
            s1 = scores_full.get(name)
            if s1 is not None and s1.shape == s2.shape:
                final_scores[name] = torch.minimum(
                    s1.detach().to(device=s2.device).float(),
                    s2.detach().float(),
                )
            else:
                final_scores[name] = s2

        first_prune = float(first_stats.get("actual_prune_fraction_eligible", 0.0))
        second_prune = float(second_stats.get("actual_prune_fraction_eligible", 0.0))
        score_stability = _snapshot_similarity_local(scores_masked, scores_full)

        round_first_pass_prune.append(first_prune)
        round_second_pass_prune.append(second_prune)
        round_intersection_prune.append(actual_prune_fraction)
        round_score_stability.append(score_stability)
        round_probe_confidences_first.append(float(meta_full.get("probe_confidence", 1.0)))
        round_probe_confidences_second.append(float(meta_masked.get("probe_confidence", 1.0)))

        final_stats = {
            "eligible_parameter_count": float(eligible_total),
            "retained_eligible_parameter_count": float(retained_eligible),
            "pruned_eligible_parameter_count": float(max(0, eligible_total - retained_eligible)),
            "actual_prune_fraction_eligible": actual_prune_fraction,
            "target_prune_fraction": float(target_prune_fraction),
            "final_round_prune_fraction": float(round_prune_fraction),
            "first_pass_actual_prune_fraction_eligible": first_prune,
            "second_pass_actual_prune_fraction_eligible": second_prune,
            "intersection_actual_prune_fraction_eligible": actual_prune_fraction,
            "connectivity_restored_coordinate_count": float(restored_total),
            "connectivity_closure": bool(_cfg_value(cfg, "connectivity_closure", False)),
            "min_connections_per_unit": int(_cfg_value(cfg, "min_connections_per_unit", 1)),
            "pruning_strategy": str(_cfg_value(cfg, "pruning_strategy", "sensitivity")).lower(),
            "sensitivity_pruning_two_pass": True,
            "cumulative_iterative_pruning": True,
        }

        # Make pruning cumulative for the next round.
        active_masks = candidate_masks
        apply_masks_(working_model, active_masks)

        # Stop once the requested target has been reached. Intersection can
        # overshoot the nominal round fraction, so this check matters.
        if actual_prune_fraction >= target_prune_fraction:
            early_stop_triggered = round_idx + 1 < total_rounds
            break

    # Apply only the final mask to the original model.
    apply_masks_(model, final_masks)

    final_stats["sensitivity_prune_rounds_completed"] = float(len(round_fractions))
    final_stats["round_prune_fraction_schedule"] = [float(x) for x in round_fractions]
    final_stats["round_first_pass_prune_schedule"] = [float(x) for x in round_first_pass_prune]
    final_stats["round_second_pass_prune_schedule"] = [float(x) for x in round_second_pass_prune]
    final_stats["round_intersection_prune_schedule"] = [float(x) for x in round_intersection_prune]
    final_stats["round_score_stability_schedule"] = [float(x) for x in round_score_stability]
    final_stats["round_probe_confidence_first_schedule"] = [
        float(x) for x in round_probe_confidences_first
    ]
    final_stats["round_probe_confidence_second_schedule"] = [
        float(x) for x in round_probe_confidences_second
    ]
    final_stats["sensitivity_prune_early_stop_triggered"] = bool(early_stop_triggered)
